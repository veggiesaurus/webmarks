                        ...
                        {   
                            ...
                            {                 
                                ...
                                if (rowsMarks && rowsMarks.length)
                                {
                                    ...
                                    {
                                        if (errAverages) throw errAverages;
                                        if (rowsAverages && rowsAverages.length)
                                        {
                                            ...
                                            {
                                                if (errHists) throw errHists;
                                                if (rowsHists && rowsHists.length)
                                                    res.render("index", {courseCode:urlCourseCode, courseName:rows[0].courseName, numCategories:rows[0].numCategories, updateDate:rows[0].updateDate.toDateString(), cats:rowsCats, averages:rowsAverages,studentID:rowsStudent[0].studentID, studentName:rowsStudent[0].name, marks:rowsMarks, hists:JSON.stringify(rowsHists)});
                                                else
                                                    res.render("index", {courseCode:urlCourseCode, courseName:rows[0].courseName, numCategories:rows[0].numCategories, updateDate:rows[0].updateDate.toDateString(), cats:rowsCats, averages:rowsAverages,studentID:rowsStudent[0].studentID, studentName:rowsStudent[0].name, marks:rowsMarks, hists:-1});
